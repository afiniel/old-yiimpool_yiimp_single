...
echo -e "$YELLOW Generating Certbot Request for $RED ${DomainName} $COL_RESET ...$COL_RESET"
...
restart_service php7.3-fpm >/dev/null 2>&1
  ...
  echo -e "$RED "Certbot generation failed, after the installer is finished check $GREEN /var/log/letsencrypt $COL_RESET (must be root to view) on why it failed.$COL_RESET"
fi
set +eu +o pipefail
